 ...
 *  Raspberry PI port by Donkey Kong
 *
...
static T bitRead(T val, U bit)
{
  return (val >> bit) & ((T)0x1);
}
 ...
 *         optional wire
 */
Adafruit_SHT31::Adafruit_SHT31() : I2CDevice(SHT31_DEFAULT_ADDR)
{
...
{
  uint8_t data[3];
  readI2C(SHT31_READSTATUS, data, 3, 0);
...
void Adafruit_SHT31::readBoth(float *temperature_out, float *humidity_out)
{
  if (!readTempHum())
  {
    *temperature_out = *humidity_out = std::numeric_limits<float>::quiet_NaN();
    return;
  }
...
bool Adafruit_SHT31::readTempHum(void)
{
  ...
  readI2C(SHT31_MEAS_HIGHREP, readbuffer, 6, 20.0);
